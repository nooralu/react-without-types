...
export default class ProfilerStore extends EventEmitter {
  ...
  _isProfilingBasedOnUserInput = false;
  ...
  onProfilingStatus = (isProfiling) => {
    ...
    if (this._isProfilingBasedOnUserInput !== isProfiling) {
      logEvent({
        event_name: "error",
        error_message:
          `Unexpected profiling status. Expected ${this._isProfilingBasedOnUserInput.toString()}, but received ${isProfiling.toString()}.`,
