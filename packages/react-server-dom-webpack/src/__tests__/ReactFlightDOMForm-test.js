...
describe("ReactFlightDOMForm", () => {
  ...
  it("can submit a multiple complex closure server action without hydrating it", async () => {
    ...
    const { returnValue } = await submit(
      form.getElementsByTagName("button")[1],
    );
      ...
      async function action(prevState, formData) {
        return {
          count: prevState.count +
            parseInt(formData.get("incrementAmount"), 10),
        };
      },
    );
  ...
  it("useActionState can return binary state during MPA form submission", async () => {
    ...
    async function submitTheForm() {
      ...
      const postbackResponse = await ReactServerDOMClient
        .createFromReadableStream(postbackRscStream, {
          serverConsumerManifest: {
            moduleMap: null,
            moduleLoading: null,
          },
