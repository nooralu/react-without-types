    ...
    return (
      <ul className={styles.Hook}>
        <li>
          ...
          <span
            className={hook.id !== null
              ? styles.PrimitiveHookName
              : styles.Name}
          >
            {hook.name}
...
const HookChangeSummary = memo(
  ({ hooks, fiberID, state, displayMode = "detailed" }) => {
    const { parseHookNames, toggleParseHookNames, inspectedElement } =
      useContext(
    ...
    const hookNames = element != null
      ? getAlreadyLoadedHookNames(element)
      : null;
