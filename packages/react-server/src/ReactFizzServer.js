...
function RequestInstance(
  ...
  this.progressiveChunkSize = progressiveChunkSize === undefined
  ...
  if (debugInfo != null) {
    for (let i = debugInfo.length - 1; i >= 0; i--) {
      ...
      if (info.awaited != null) {
        ...
        const bestStack = asyncInfo.debugStack == null
          ? asyncInfo.awaited
          : asyncInfo;
...
function logRecoverableError(
  ...
  const errorDigest = __DEV__ && debugTask
...
function renderSuspenseList(
  ...
  if (
    ...
    if (
      ...
      const iterator = children[
        ASYNC_ITERATOR
      ]();
      if (iterator) {
        if (__DEV__) {
          validateAsyncIterable(task, children, -1, iterator);
        }
...
function validateIterable(
  ...
  if (__DEV__) {
    if (iterator === iterable) {
      ...
      // as needed.
      const isGeneratorComponent = childIndex === -1 && // Only the root child is valid
  ...
  if (typeof node === "object") {
    switch (node.$$typeof) {
      case REACT_ELEMENT_TYPE: {
        ...
        const keyOrIndex = key == null
          ? (childIndex === -1 ? 0 : childIndex)
          : key;
    ...
    if (
      ...
      if (iterator) {
        if (__DEV__) {
          validateAsyncIterable(task, node, childIndex, iterator);
        }
    ...
    try {
      ...
      if (request.status === ABORTING) {
        ...
        if (typeof x.then === "function") {
          const wakeable = x;
          const thenableState = thrownValue === SuspenseException
            ? getThenableStateAfterSuspending()
            : null;
  ...
  try {
    ...
    if (typeof x === "object" && x !== null) {
      ...
      if (typeof x.then === "function") {
        ...
        task.thenableState = thrownValue === SuspenseException
          ? getThenableStateAfterSuspending()
          : null;
...
function flushCompletedQueues(
  ...
  try {
    ...
    if (completedRootSegment !== null) {
      ...
      request.completedRootSegment = null;
      const isComplete = request.allPendingTasks === 0 &&
...
export function prepareForStartFlowingIfBeforeAllReady(request) {
  const shellComplete = request.trackedPostpones === null
