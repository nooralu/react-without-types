...
describe("Store", () => {
  ...
  const { render, unmount, createContainer } =
  ...
  it("should handle reorder of filtered elements", async () => {
    ...
    await actAsync(
      async () => (store.componentFilters = [
        createDisplayNameFilter("^IgnoreMe", true),
      ]),
    );
  ...
  it("should show the right display names for special component types", async () => {
    ...
    MemoizedFakeHigherOrderComponentWithDisplayNameOverride.displayName =
      "memoRefOverride";
    const ForwardRefFakeHigherOrderComponentWithDisplayNameOverride = React
      .forwardRef(FakeHigherOrderComponent);
