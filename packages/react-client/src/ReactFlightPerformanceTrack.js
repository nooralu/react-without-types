...
export function logComponentRender(
  ...
  if (supportsUserTiming && childrenEndTime >= 0 && trackIdx < 10) {
    ...
    const color = selfTime < 0.5
      ? isPrimaryEnv ? "primary-light" : "secondary-light"
      : selfTime < 50
      ? isPrimaryEnv ? "primary" : "secondary"
      : selfTime < 500
      ? isPrimaryEnv ? "primary-dark" : "secondary-dark"
      : "error";
    const entryName = isPrimaryEnv || env === undefined
      ? name
      : name + " [" + env + "]";
...
export function logComponentAwaitAborted(
  ...
  if (supportsUserTiming && endTime > 0) {
    const entryName = "await " +
      getIOShortName(asyncInfo.awaited, "", asyncInfo.env, rootEnv);
