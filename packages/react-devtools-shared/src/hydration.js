...
function createDehydrated(
  ...
  const dehydrated = {
    ...
    name: typeof data.constructor !== "function" ||
  ...
  switch (type) {
    ...
    case "function":
      ...
      return {
        ...
        preview_short: formatDataForPreview(data, false),
        preview_long: formatDataForPreview(data, true),
        name: typeof data.name === "function" || !data.name
          ? "function"
          : data.name,
        type,
      };
    ...
    case "react_lazy": {
      ...
      if (level >= LEVEL_THRESHOLD && !isPathAllowedCheck) {
        cleaned.push(path);
        const inspectable = payload !== null &&
    ...
    case "iterator":
      ...
      if (level >= LEVEL_THRESHOLD && !isPathAllowedCheck) {
        ...
        Array.from(data).forEach(
          (item, i) => (unserializableValue[i] = dehydrate(
            item,
            cleaned,
            unserializable,
            path.concat([i]),
    ...
    case "thenable":
      ...
      if (level >= LEVEL_THRESHOLD && !isPathAllowedCheck) {
        cleaned.push(path);
        return {
          inspectable: data.status === "fulfilled" ||
