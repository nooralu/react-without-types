...
function AutoSizedCanvas({
  ...
  useLayoutEffect(() => {
    ...
    const componentMeasureSearchResult = searchResults.length > 0
    ...
    if (componentMeasureSearchResult != null) {
      const scrollState = moveStateToRange({
        ...
        rangeEnd: componentMeasureSearchResult.timestamp +
  ...
  return (
    <Fragment>
      ...
      <CanvasPageContextMenu
        ...
        viewState={viewState}
        canvasWidth={width}
        closedMenuStub={!surfaceRef.current.hasActiveView()
          ? (
            <EventTooltip
              canvasRef={canvasRef}
              data={data}
