...
import { isAwaitInUserspace, resolveRequest } from "./ReactFlightServer";
...
const pendingOperations = __DEV__ && enableAsyncDebugInfo ? new Map() : (null);

// Keep the last resolved await as a workaround for async functions missing data.
let lastRanAwait = null;

function resolvePromiseOrAwaitNode(
  unresolvedNode,
  endTime,
) {
  const resolvedNode = unresolvedNode;
        ...
        if (type === "PROMISE") {
          ...
          if (currentAsyncId !== triggerAsyncId) {
            ...
            node = {
              ...
              stack: owner === null
                ? null
                : parseStackTracePrivate(new Error(), 5),
              start: performance.now(),
              end: -1.1, // Set when we resolve.
              promise: new WeakRef(resource),
              awaited: trigger === undefined
          ...
          if (trigger === undefined) {
            ...
            node = {
              ...
              owner: owner,
              stack: owner === null
                ? parseStackTracePrivate(new Error(), 3)
                : null,
              start: performance.now(),
