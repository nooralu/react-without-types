...
describe("ReactFabric", () => {
  beforeEach(() => {
    ...
    require(
      "react-native/Libraries/ReactPrivate/InitializeNativeFabricUIManager",
    );

    React = require("react");
    StrictMode = React.StrictMode;
    ReactFabric = require("react-native-renderer/fabric");
    ReactNativePrivateInterface = require(
 ...
 RCTText {"foo":"a","bar":"b"}
   RCTRawText {"text":"1"}`);
    ...
    const argIndex =
      gate((flags) => flags.passChildrenWhenCloningPersistedNodes) ? 2 : 1;
    expect(
 ...
 RCTView {}
  ...
  it("dispatches event with target as instance", async () => {
    ...
    function getViewById(id) {
      const [reactTag, , , , instanceHandle] = nativeFabricUIManager.createNode
        .mock.calls.find(
  ...
  it("getPublicInstanceFromInternalInstanceHandle should provide public instances for HostComponent", async () => {
    ...
    const publicInstance = ReactFabric
    ...
    const publicInstanceAfterUnmount = ReactFabric
      .getPublicInstanceFromInternalInstanceHandle(
        internalInstanceHandle,
      );
