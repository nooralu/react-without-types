...
export default function Element({ data, index, style }) {
  ...
  const { ownerFlatTree, ownerID, inspectedElementID } = useContext(
    TreeStateContext,
  );
  const dispatch = useContext(TreeDispatcherContext);

  const element = ownerFlatTree !== null
    ? ownerFlatTree[index]
  ...
  const isDescendantOfSelected = inspectedElementID !== null &&
    !isSelected &&
    ...
    >
      ...
      <div className={styles.Wrapper}>
        ...
        {errorCount > 0 && (
          <Icon
            type="error"
            className={isSelected && treeFocused
