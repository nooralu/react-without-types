...
function calculateElementOffset(elementDepth) {
...
export default function Tree() {
  ...
  const setListDOMElementRef = useCallback((listDOMElement) => {
    ...
    const isElementFullyVisible = elementLeft >= viewportLeft &&
      elementRight <= viewportRight;

    if (!isElementFullyVisible) {
      const horizontalDelta = Math.min(0, elementLeft - viewportLeft) +
  ...
  useEffect(() => {
    ...
    const isElementFullyVisible = elementTop >= viewportTop &&
      elementBottom <= viewportBottom &&
      elementLeft >= viewportLeft &&
      elementRight <= viewportRight;

    if (!isElementFullyVisible) {
      const verticalDelta = Math.min(0, elementTop - viewportTop) +
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event) => {
      ...
      switch (event.key) {
        ...
        case "ArrowLeft":
          event.preventDefault();
          element = inspectedElementID !== null
...
function VerticalDelimiter() {
  ...
  const indexOfLowestDescendant = store.getIndexOfLowestDescendantElement(
    element,
  );
  ...
  const delimiterHeight = (indexOfLowestDescendant + 1) * lineHeight -
    delimiterTop;
