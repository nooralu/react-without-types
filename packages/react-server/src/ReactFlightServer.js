...
import {
  resolveOwner,
  setCurrentOwner,
} from "./flight/ReactFlightCurrentOwner";
...
import { getSuspendedThenable, SuspenseException } from "./ReactFlightThenable";
...
const doNotLimit = __DEV__ ? new WeakSet() : (null);
...
const defaultPostponeHandler = noop;
...
function RequestInstance(
  ...
  this.onPostpone = onPostpone === undefined
    ? defaultPostponeHandler
    : onPostpone;
  ...
  if (__DEV__) {
    ...
    this.environmentName = environmentName === undefined
      ? () => "Server"
      : typeof environmentName !== "function"
      ? () => environmentName
      : environmentName;
    this.filterStackFrame = filterStackFrame === undefined
...
function serializeThenable(
  ...
  const newTask = createTask(
    request,
    thenable, // will be replaced by the value before we retry. used for debug info.
...
function serializeReadableStream(
  ...
  const startStreamRow = streamTask.id.toString(16) + ":" +
    (supportsBYOB ? "r" : "R") + "\n";
...
function serializeAsyncIterable(
  ...
  if (__DEV__) {
    const debugInfo = iterable._debugInfo;
    if (debugInfo) {
    ...
    if (entry.done) {
      ...
      if (entry.value === undefined) {
        ...
        try {
          ...
          endStreamRow = streamTask.id.toString(16) +
            ":C" +
...
function processServerComponentReturnValue(
  ...
  if (
    typeof result[ASYNC_ITERATOR] === "function" &&
...
function renderFunctionComponent(
  ...
  if (__DEV__ || (enableProfilerTimer && enableAsyncDebugInfo)) {
    ...
    if (trackedThenables !== null) {
      const stacks = __DEV__ && enableAsyncDebugInfo
        ? trackedThenables._stacks ||
          (trackedThenables._stacks = [])
        : (null);
...
function renderFragment(
  ...
  if (__DEV__) {
    const debugInfo = children._debugInfo;
    if (debugInfo) {
    ...
    case AWAIT_NODE: {
      ...
      if (awaited !== null) {
        ...
        if (ioNode === undefined) {
          ...
          const startTime = node.start;
          const endTime = node.end;
          if (endTime <= request.timeOrigin) {
            ...
            if (
              ...
              const promise = awaited.promise === null
                ? undefined
            ...
            if (
              ...
              emitDebugChunk(request, task.id, {
                awaited: ioNode, // This is deduped by this reference.
                env: env,
                owner: node.owner,
...
function emitAsyncSequence(
  ...
  if (awaitedNode === undefined) {
    ...
    const debugInfo = {
      awaited: awaitedNode, // This is deduped by this reference.
      env: env,
    };
    if (__DEV__) {
...
function serializeServerReference(
  ...
  const serverReferenceMetadata = __DEV__ && location !== null
    ? {
      id,
      bound,
...
function renderModel(
  ...
  try {
    ...
    const wasReactNode = typeof model === "object" &&
      model !== null &&
  ...
  if (typeof value === "object") {
    switch (value.$$typeof) {
      ...
      case REACT_LAZY_TYPE: {
        ...
        if (__DEV__) {
          const debugInfo = lazy._debugInfo;
          if (debugInfo) {
            // If this came from Flight, forward any debug info into this new row.
            if (!canEmitDebugInfo) {
              // We don't have a chunk to assign debug info. We need to outline this
...
function serializeErrorValue(request, error) {
  if (__DEV__) {
    ...
    try {
      ...
      const errorEnv = error.environmentName;
...
function emitDebugChunk(
  ...
  const json = serializeDebugModel(request, 500, debugInfo);
  if (request.debugDestination !== null) {
    ...
    const row = serializeRowHeader("D", id) + '"$' + outlinedId.toString(16) +
      '"\n';
...
function emitIOInfoChunk(
  ...
  const json = serializeDebugModel(request, objectLimit, debugIOInfo);
...
function serializeIONode(
  ...
  const endTime = ioNode.tag === UNRESOLVED_PROMISE_NODE
...
const CONSTRUCTOR_MARKER = __DEV__ ? Symbol() : (null);

let debugModelRoot = null;
let debugNoOutline = null;
...
function renderDebugModel(
  ...
  if (typeof value === "object") {
    ...
    if (proto !== ObjectPrototype && proto !== null) {
      const object = value;
      const instanceDescription = Object.create(null);
  ...
  if (typeof value === "function") {
    ...
    // $FlowFixMe[method-unbinding]
    const functionBody = Function.prototype.toString.call(value);
    ...
    if (thenable !== null) {
      ...
      if (sequence !== null) {
        ...
        if (node.tag === UNRESOLVED_PROMISE_NODE) {
          ...
          const env = (0, request.environmentName)();
          const asyncInfo = {
            awaited: node, // This is deduped by this reference.
            env: env,
          };
          // We don't have a start time for this await but in case there was no start time emitted
          // we need to include something. TODO: We should maybe ideally track the time when we
...
export function resolveDebugMessage(request, message) {
  if (!__DEV__) {
    // These errors should never make it into a build so we don't need to encode them in codes.json
    // eslint-disable-next-line react-internal/prod-error-codes
    throw new Error(
...
export function closeDebugChannel(request) {
  if (!__DEV__) {
    // These errors should never make it into a build so we don't need to encode them in codes.json
    // eslint-disable-next-line react-internal/prod-error-codes
    throw new Error(
