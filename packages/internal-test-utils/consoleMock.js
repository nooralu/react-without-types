...
export function assertConsoleLogsCleared() {
  ...
  if (logs.length > 0 || errors.length > 0 || warnings.length > 0) {
    let message = `${chalk.dim("asserConsoleLogsCleared")}(${
      chalk.red(
        "expected",
      )
    })\n`;
          ...
          if (
            ...
            throwFormattedError(
              `Log entries that are arrays must be of the form [string, {withoutStack: true}]` +
                `\n\nInstead received [${typeof currentExpectedMessage}, ${
                  JSON.stringify(
      ...
      if (logsMismatchingFormat.length > 0) {
        throwFormattedError(
          logsMismatchingFormat
            .map(
              (item) =>
                `Received ${item.args.length} arguments for a message with ${item.expectedArgCount} placeholders:\n  ${
