...
function aquireHostResource(
  ...
  if (hostInstance) {
    const publicInstance = getPublicInstance(hostInstance);
    let resourceInstances = hostResourceToDevToolsInstanceMap.get(
      publicInstance,
    );
...
function releaseHostResource(
  ...
  if (hostInstance) {
    const publicInstance = getPublicInstance(hostInstance);
    const resourceInstances = hostResourceToDevToolsInstanceMap.get(
      publicInstance,
    );
    if (resourceInstances !== undefined) {
...
export function attach(
  ...
  const supportsTogglingError = typeof setErrorHandler === "function" &&
  ...
  const fiberToComponentLogsMap = new WeakMap();
  ...
  function debug(
    ...
    if (__DEBUG__) {
      const displayName = instance.kind === VIRTUAL_INSTANCE
        ? instance.data.name || "null"
        : instance.data.tag +
          ":" +
          (getDisplayNameForFiber(instance.data) || "null");

      const maybeID = instance.kind === FILTERED_FIBER_INSTANCE
        ? "<no id>"
        : instance.id;

      const parentDisplayName = parentInstance === null
        ? ""
      ...
      const maybeParentID = parentInstance === null ||
  ...
  function debugTree(instance, indent = 0) {
    if (__DEBUG__) {
      const name = (instance.kind !== VIRTUAL_INSTANCE
  ...
  const hideElementsWithEnvs = new Set();
  ...
  function getContextChanged(prevFiber, nextFiber) {
    let prevContext = prevFiber.dependencies &&
      prevFiber.dependencies.firstContext;
    let nextContext = nextFiber.dependencies &&
      nextFiber.dependencies.firstContext;
  ...
  const pendingSuspenderChanges = new Set();
  ...
  function recordConsoleLogs(
    ...
    if (componentLogsEntry === undefined) {
      ...
      const totalCount = componentLogsEntry.errorsCount +
        componentLogsEntry.warningsCount;
  ...
  function recordReconnect(
    ...
    if (isRoot) {
      ...
      const nameProp = fiber.tag === SuspenseComponent
  ...
  function recordVirtualReconnect(
    ...
    const key = typeof componentInfo.key === "string"
      ? componentInfo.key
      : null;
  ...
  function recordSuspenseMount(
    ...
    const unfilteredParentInstance = unfilteredParent !== null
      ? unfilteredParent.instance
      : null;
    if (
    ...
    const parentID = unfilteredParentInstance === null
      ? 0
      : unfilteredParentInstance.id;
  ...
  function removePreviousSuspendedBy(
    ...
    const suspenseNode = instance.suspenseNode === null
      ? parentSuspenseNode
  ...
  function mountVirtualInstanceRecursively(
    virtualInstance,
    firstChild,
    lastChild, // non-inclusive
    traceNearestHostComponentUpdate,
    virtualLevel, // the nth level of virtual instances
  ) {
  ...
  function trackDebugInfoFromUsedThenables(fiber) {
    ...
    for (let i = 0; i < usedThenables.length; i++) {
      const thenable = usedThenables[i];
  ...
  const hostAsyncInfoCache = new WeakMap();
  ...
  function trackDebugInfoFromHostResource(
    ...
    const mayResourceSuspendCommit = resource.type === "stylesheet" &&
  ...
  function trackDebugInfoFromHostComponent(
    ...
    const maySuspendCommit = type === "img" &&
      props.src != null &&
      props.src !== "" &&
      props.onLoad == null &&
      props.loading !== "lazy";
  ...
  function updateFiberRecursively(
    fiberInstance, // null if this should be filtered
    nextFiber,
    prevFiber,
    traceNearestHostComponentUpdate,
  ) {
    ...
    try {
      ...
      const isLegacySuspense = nextFiber.tag === SuspenseComponent &&
        OffscreenComponent === -1;
      const prevDidTimeout = isLegacySuspense &&
        prevFiber.memoizedState !== null;
      const nextDidTimeOut = isLegacySuspense &&
      ...
      if (fiberInstance !== null) {
        ...
        if (fiberInstance.kind === FIBER_INSTANCE) {
          ...
          const isProfilingSupported = nextFiber.hasOwnProperty(
            "treeBaseDuration",
          );
          if (isProfilingSupported) {
  ...
  function instanceToSerializedElement(
    ...
    if (instance.kind === FIBER_INSTANCE) {
      ...
      return {
        ...
        key: fiber.key,
        env: null,
        stack: fiber._debugOwner == null || fiber._debugStack == null
          ? null
      ...
      return {
        ...
        stack: componentInfo.owner == null || componentInfo.debugStack == null
          ? null
  ...
  function getSuspendedByOfSuspenseNode(
    suspenseNode,
    filterByChildInstance, // only include suspended by instances in this subtree
  ) {
  ...
  function serializeAsyncInfo(
    ...
    let awaitStack = asyncInfo.debugStack == null
      ? null
    ...
    if (
      ...
      awaitStack = null;
      awaitOwnerInstance = parentInstance.kind === FILTERED_FIBER_INSTANCE
        ? null
        : parentInstance;
      if (
    ...
    return {
      awaited: {
        ...
        owner: ioOwnerInstance === null
          ? null
          : instanceToSerializedElement(ioOwnerInstance),
        stack: ioInfo.debugStack == null
          ? null
          // While we have a ReactStackTrace on ioInfo.stack, that will point to the location on
          // the server. We need a location that points to the virtual source on the client which
          // we can then use to source map to the original location.
  ...
  function inspectFiberInstanceRaw(
    ...
    const usesHooks = (tag === FunctionComponent ||
    ...
    const showState = tag === ClassComponent ||
    ...
    if (
      ...
      if (stateNode && stateNode.context != null) {
        ...
        const shouldHideContext = elementType === ElementTypeClass &&
      ...
      while (current !== null) {
        ...
        if (
          ...
          // PR #12501 changed it to "_context" for 16.3.1+
          // NOTE Keep in sync with getDisplayNameForFiber()
          const providerResolvedContext = currentType._context ||
            currentType.context;
    ...
    const isTimedOutSuspense = tag === SuspenseComponent &&
      memoizedState !== null;
    ...
    const suspendedBy = fiberInstance.suspenseNode !== null
    ...
    return {
      ...
      errors: componentLogsEntry === undefined
        ? []
        : Array.from(componentLogsEntry.errors.entries()),
      warnings: componentLogsEntry === undefined
        ? []
    ...
    try {
      ...
      if (error.name === "ReactDebugToolsUnsupportedHookError") {
        return {
          ...
          id,
          responseID: requestID,
          message: "Unsupported hook in the react-debug-tools package: " +
            error.message,
        };
      }
      ...
      (commitProfilingMetadata, rootID) => {
        ...
        commitProfilingMetadata.forEach((commitProfilingData, commitIndex) => {
          ...
          commitData.push({
            ...
            effectDuration: effectDuration !== null
              ? formatDurationToMicrosecondsGranularity(effectDuration)
              : null,
            fiberActualDurations,
            fiberSelfDurations,
            passiveEffectDuration: passiveEffectDuration !== null
  ...
  function getSourceForFiberInstance(
    ...
    const dispatcherRef = getDispatcherRef(renderer);
    const stackFrame = dispatcherRef == null ? null : getSourceLocationByFiber(
      ReactTypeOfWork,
      fiberInstance.data,
      dispatcherRef,
    );
  ...
  function getSourceForInstance(
    ...
    if (typeof unresolvedSource === "string") {
      ...
      const lastLine = idx === -1
        ? unresolvedSource
        : unresolvedSource.slice(idx + 1);
  ...
  if (__IS_INTERNAL_MCP_BUILD__) {
    ...
    function __internal_only_getComponentTree() {
      let treeString = "";
