  ...
  if (typeof value === "object" && value !== null) {
    ...
    if (Array.isArray(value)) {
      for (let i = 0; i < value.length; i++) {
        const childRef = reference !== undefined
          ? reference + ":" + i
          : undefined;
...
function initializeModelChunk(chunk) {
  ...
  const rootReference = chunk.reason === -1
    ? undefined
...
function parseTypedArray(
  ...
  const promise = constructor === ArrayBuffer
  ...
  if (value[0] === "$") {
    switch (value[1]) {
      ...
      case "F": {
        ...
        const metaData = getOutlinedModel(response, ref, obj, key, createModel);
