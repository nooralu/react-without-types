...
import {
  ...
  completeBoundaryUpgradeToViewTransitions
    as upgradeToViewTransitionsInstruction,
...
export function createRenderState(
  ...
  const inlineScriptWithNonce = nonceScript === undefined
    ? startInlineScript
    : stringToPrecomputedChunk(
      '<script nonce="' + escapeTextForBrowser(nonceScript) + '"',
    );
  const nonceStyle = typeof nonce === "string"
    ? undefined
    : nonce && nonce.style;
  const inlineStyleWithNonce = nonceStyle === undefined
    ? startInlineStyle
    : stringToPrecomputedChunk(
...
export function createRootFormatContext(namespaceURI) {
  const insertionMode = namespaceURI === "http://www.w3.org/2000/svg"
    ? SVG_MODE
...
const formReplayingRuntimeScript = stringToPrecomputedChunk(
  formReplayingRuntime,
);
      ...
      if (__DEV__) {
        if (typeof precedence === "string") {
          if (props.disabled != null) {
            ...
            const propDescription = props.onLoad && props.onError
  ...
  if (__DEV__) {
    if (hasOwnProperty.call(props, "children")) {
      ...
      if (
        ...
        const childType = typeof child === "function"
          ? "a Function"
          : typeof child === "symbol"
          ? "a Sybmol"
          : "an Array";
...
function pushImg(
  ...
  const pictureOrNoScriptTagInScope = formatContext.tagScope &
  ...
  if (
    ...
    if (hoistableState !== null) {
      ...
      // for SSR reveals.
      const isInSuspenseWithEnterViewTransition = formatContext.tagScope &
        APPEARING_SCOPE;
...
function pushScriptImpl(
  ...
  if (__DEV__) {
    if (children != null && typeof children !== "string") {
      const descriptiveStatement = typeof children === "number"
...
const clientRenderedSuspenseBoundaryError1D = stringToPrecomputedChunk(
  ' data-cstck="',
);
...
export function writeCompletedSegmentInstruction(
  ...
  const scriptFormat = !enableFizzExternalRuntime ||
    resumableState.streamingFormat === ScriptStreamingFormat;
  if (scriptFormat) {
    writeChunk(destination, renderState.startInlineScript);
    writeChunk(destination, endOfStartTag);
...
const clientRenderScriptFunctionOnly = stringToPrecomputedChunk(
...
function preconnect(href, crossOrigin) {
  ...
  if (typeof href === "string" && href) {
    const bucket = crossOrigin === "use-credentials"
      ? "credentials"
  ...
  if (as && href) {
    switch (as) {
      ...
      case "style": {
        ...
        resumableState.styleResources[key] = options &&
      ...
      case "script": {
        ...
        resumableState.scriptResources[key] = options &&
            (typeof options.crossOrigin === "string" ||
              typeof options.integrity === "string")
          ? [options.crossOrigin, options.integrity]
          : PRELOAD_NO_CREDS;
  ...
  if (href) {
    ...
    switch (as) {
      case "script": {
        ...
        resource = [];
        resumableState.moduleScriptResources[key] = options &&
            (typeof options.crossOrigin === "string" ||
              typeof options.integrity === "string")
          ? [options.crossOrigin, options.integrity]
          : PRELOAD_NO_CREDS;
      ...
      default: {
        const hasAsType = resumableState.moduleUnknownResources.hasOwnProperty(
          as,
        );
        let resources;
        if (hasAsType) {
