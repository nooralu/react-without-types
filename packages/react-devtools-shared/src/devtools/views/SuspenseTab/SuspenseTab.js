...
import { OptionsContext, StoreContext } from "../context";
...
function ToggleUniqueSuspenders() {
  ...
  function handleToggleUniqueSuspenders() {
    ...
    const nextTimeline = rootID === null
      ? []
...
function ToggleInspectedElement({
  ...
  return (
    <Button
      ...
      title={state.inspectedElementHidden
...
function SuspenseTab(_) {
  ...
  const onResizeTree = (event) => {
    ...
    const currentMousePosition = orientation === "horizontal"
      ? event.clientX - left
      : event.clientY - top;
    ...
    const isMousePositionInBounds = currentMousePosition > boundaryMin &&
      currentMousePosition < boundaryMax;

    if (isMousePositionInBounds) {
      const resizedElementDimension = orientation === "horizontal"
        ? width
        : height;
      const actionType = orientation === "horizontal"
...
export default (portaledContent(SuspenseTab));
