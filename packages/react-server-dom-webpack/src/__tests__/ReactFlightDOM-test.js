...
describe("ReactFlightDOM", () => {
  ...
  it("should error when a bundler uses async ESM modules with createClientModuleProxy", async () => {
    ...
    const errorMessage = `The module "${
      Object.keys(webpackMap).at(0)
  ...
  it("should handle streaming async server components", async () => {
    ...
    await serverAct(async () => {
      await act(async () => {
        suspendedChunk.resolve({
          value,
          done: false,
          next: donePromise.promise,
        });
  ...
  it("should support float methods when rendering in Fizz", async () => {
    ...
    function getResponse() {
      if (response === null) {
        response = ReactServerDOMClient.createFromReadableStream(
          flightReadable,
        );
      }
      return response;
  ...
  it("supports Float hints from concurrent Flight -> Fizz renders", async () => {
    ...
    const { writable: fizzWritable1, readable: fizzReadable1 } =
      getTestStream();
    const { writable: fizzWritable2, readable: fizzReadable2 } =
      getTestStream();
    await serverAct(async () => {
