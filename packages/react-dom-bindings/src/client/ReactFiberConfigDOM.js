...
export const scheduleTimeout = typeof setTimeout === "function"
  ? setTimeout
  : undefined;
export const cancelTimeout = typeof clearTimeout === "function"
  ? clearTimeout
  : undefined;
...
export function unhideInstance(instance, props) {
  ...
  const display = styleProp !== undefined &&
      styleProp !== null &&
  ...
  if (__DEV__) {
    ...
    outer: while (nextNode != null) {
      ...
      if (
        ...
        const fiber = getInstanceFromNode(node) ||
...
export function restoreViewTransitionName(
  ...
  const style = instance.style;
  const styleProp = props[STYLE];
  const viewTransitionName = styleProp != null
  ...
  const viewTransitionClass = styleProp != null
  ...
  if (style.display === "inline-block") {
    ...
    if (styleProp == null) {
      ...
      style.display = display == null || typeof display === "boolean"
        ? ""
        : display;
      ...
      if (margin != null) {
        ...
        style.marginTop = marginTop == null || typeof marginTop === "boolean"
          ? ""
          : marginTop;
...
export function cancelRootViewTransitionName(rootContainer) {
  const documentElement = rootContainer.nodeType === DOCUMENT_NODE
...
export function restoreRootViewTransitionName(rootContainer) {
  ...
  const documentElement = containerInstance.ownerDocument.documentElement;
  if (
    documentElement !== null &&
    // $FlowFixMe[prop-missing]
    documentElement.style.viewTransitionName === "none"
...
function getComputedTransform(style) {
  ...
  if (scale !== "none" && scale !== "") {
    ...
    transform = (parts.length === 3 ? "scale3d" : "scale") +
      "(" +
      parts.join(", ") +
      ") " +
      transform;
  ...
  if (rotate !== "none" && rotate !== "") {
    ...
    if (parts.length === 1) {
      ...
      transform = "rotate" + parts[0].toUpperCase() + "(" + parts[1] + ") " +
        transform;
    } else {
  ...
  if (translate !== "none" && translate !== "") {
    ...
    transform = (parts.length === 3 ? "translate3d" : "translate") +
      "(" +
      parts.join(", ") +
      ") " +
      transform;
...
export function startViewTransition(
  ...
  const ownerDocument = rootContainer.nodeType === DOCUMENT_NODE
    ? rootContainer
  ...
  try {
    ...
    const transition = ownerDocument.startViewTransition({
      update() {
        ...
        const pendingNavigation = ownerWindow.navigation &&
        ...
        if (blockingPromises.length > 0) {
          if (enableProfilerTimer) {
            const blockedReason = blockingIndexSnapshot > 0
              ? blockingPromises.length > blockingIndexSnapshot
...
export function startGestureTransition(
  ...
  try {
    ...
    const readyCallback = () => {
      ...
      const durationToRangeMultipler = (rangeEnd - rangeStart) /
        longestDuration;
      for (let i = 0; i < animations.length; i++) {
        const anim = animations[i];
        ...
        if (
          ...
          if (pseudoElement.startsWith("::view-transition-group")) {
            ...
            if (foundNews.has(groupName)) {
              ...
              isGeneratedGroupAnim = animationName != null &&
                // $FlowFixMe[prop-missing]
    ...
    const readyForAnimations = navigator.userAgent.indexOf("Chrome") !== -1
...
function normalizeListenerOptions(
  ...
  return `c=${opts.capture ? "1" : "0"}&o=${opts.once ? "1" : "0"}&p=${
    opts.passive ? "1" : "0"
  }`;
}
...
FragmentInstance.prototype.compareDocumentPosition = function (
  ...
  const firstInstance = getInstanceFromHostFiber(children[0]);
  ...
  const otherNodeIsBetweenFirstAndLastChildren = firstElementIsContained &&
...
let previousHydratableOnEnteringScopedSingleton = null;
...
function preload(href, as, options) {
  ...
  if (ownerDocument && href && as) {
    let preloadSelector = `link[rel="preload"][as="${
      ...
      const preloadProps = Object.assign(
        {
          ...
          href: as === "image" && options && options.imageSrcSet
            ? undefined
            : href,
          as,
        },
...
function preloadModule(href, options) {
  ...
  if (ownerDocument && href) {
    const as = options && typeof options.as === "string"
      ? options.as
      : "script";
    const preloadSelector = `link[rel="modulepreload"][as="${
      escapeSelectorAttributeValueInsideDoubleQuotes(
        as,
      )
...
function estimateImageBytes(instance) {
  ...
  const pixelRatio = typeof devicePixelRatio === "number"
    ? devicePixelRatio
    : 1;
...
export function waitForCommitToBeReady(
  ...
  if (state.count > 0 || state.imgCount > 0) {
    return (commit) => {
      ...
      const imgTimeout = state.imgBytes > estimatedBytesWithinLimit
        ? 50
