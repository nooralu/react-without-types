...
export let actingUpdatesScopeDepth = 0;

export const thrownErrors = [];
...
export async function act(scope) {
...
async function waitForTasksAndTimers(error) {
  do {
    // Wait until end of current task/microtask.
    await waitForMicrotasks();
...
export async function serverAct(scope) {
  // We require every `act` call to assert console logs
  // with one of the assertion helpers. Fails if not empty.
  assertConsoleLogsCleared();
