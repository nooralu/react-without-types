...
function loadServerReference(
  ...
  const serverReference = resolveServerReference(
  ...
  if (value[0] === "$") {
    ...
    switch (value[1]) {
      ...
      case "Y": {
        if (__DEV__) {
          if (value.length > 2) {
            const debugChannelCallback = response._debugChannel &&
              response._debugChannel.callback;
...
function ResponseInstance(
  ...
  if (__DEV__) {
    ...
    const rootOwner = ReactSharedInteralsServer === undefined ||
...
function resolveChunkDebugInfo(
  ...
  if (__DEV__ && enableAsyncDebugInfo) {
    ...
    const debugInfo = [{ awaited: streamState._debugInfo }];
    if (chunk.status === PENDING || chunk.status === BLOCKED) {
...
const fakeFunctionCache = __DEV__ ? new Map() : (null);
...
function initializeFakeTask(
  ...
  const env = debugInfo.env == null
    ? response._rootEnvironmentName
    : debugInfo.env;
  const ownerEnv = debugInfo.owner == null || debugInfo.owner.env == null
    ? response._rootEnvironmentName
    : debugInfo.owner.env;
  const ownerTask = debugInfo.owner == null
    ? null
...
function resolveDebugModel(
  ...
  const debugChunk = createResolvedModelChunk(response, json);
