...
import { CacheComponent, HostComponent, HostRoot } from "./ReactWorkTags";
import {
  FormReset,
    ...
    if (!didWarnAboutMismatchedHooksForComponent.has(componentName)) {
      ...
      if (hookTypesDev !== null) {
        ...
        for (let i = 0; i <= hookTypesUpdateIndexDev; i++) {
          ...
          const newHookName = i === hookTypesUpdateIndexDev
            ? currentHookName
            : oldHookName;
...
export function renderWithHooks(
  ...
  const shouldDoubleRenderDEV = __DEV__ &&
...
function finishRenderingHooks(
  ...
  const didRenderTooFewHooks = currentHook !== null &&
...
export function TransitionAwareHostComponent() {
  ...
  const prevResetState = currentHook !== null
...
function updateOptimisticImpl(
  ...
  const resolvedReducer = typeof reducer === "function"
    ? reducer
    : basicStateReducer;
...
function startTransition(
  ...
  if (enableTransitionTracing) {
    currentTransition.name = options !== undefined && options.name !== undefined
      ? options.name
      : null;
