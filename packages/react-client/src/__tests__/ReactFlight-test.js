...
describe("ReactFlight", () => {
  ...
  it("can transport Error objects as values", async () => {
    ...
    function ComponentClient({ prop }) {
      return `
        ...
        stack: ${
        normalizeCodeLocInfo(prop.stack).split("\n").slice(0, 2).join("\n")
      }
  ...
  it("should emit descriptions of errors in dev", async () => {
    ...
    const testCases = (
      <>
        ...
        <ClientErrorBoundary
          expectedMessage={"A React Element from an older version of React was rendered. " +
  ...
  it("can track owner for a flight response created in another render", async () => {
    ...
    const ReactNoopFlightClienOnTheServer = require(
