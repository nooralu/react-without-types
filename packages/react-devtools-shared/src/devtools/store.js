...
export default class Store extends EventEmitter {
  ...
  _cachedComponentWithErrorCount = 0;
  _cachedComponentWithWarningCount = 0;
  ...
  _supportsInspectMatchingDOMElement = false;
  _supportsClickToInspect = false;
  _supportsTimeline = false;
  _supportsTraceUpdates = false;
  // Dynamically set if the renderer supports the Suspense tab.
  _supportsSuspenseTab = false;

  _isReloadAndProfileFrontendSupported = false;
  _isReloadAndProfileBackendSupported = false;
  ...
  _shouldCheckBridgeProtocolCompatibility = false;
  _hookSettings = null;
  _shouldShowWarningsAndErrors = false;

  // Only used in browser extension for synchronization with built-in Elements panel.
  _lastSelectedHostInstanceElementId = null;
  ...
  supportsTogglingSuspense(rootID) {
  ...
  isDescendantOf(parentId, descendantId) {
    if (descendantId === 0) {
      return false;
    }
   ...
   */
  getIndexOfLowestDescendantElement(element) {
    let current = element;
    while (current !== null) {
  ...
  getSuspenseLineage(
    ...
    const lineage = [];
    let next = this.getSuspenseByID(suspenseID);
    while (next !== null) {
      if (next.parentID === 0) {
        next = null;
      } else {
   ...
   */
  getSuspenseRootIDForSuspense(id) {
    ...
    if (element !== null) {
      if (isCollapsed) {
        ...
        while (currentElement != null) {
          ...
          currentElement = currentElement.parentID !== 0
  ...
  onBridgeOperations = (operations) => {
    ...
    const removedSuspenseIDs = new Map();
      ...
      switch (operation) {
        case TREE_OPERATION_ADD: {
          ...
          if (type === ElementTypeRoot) {
            ...
            const isStrictModeNonCompliant = !isStrictModeCompliant &&
              supportsStrictMode;
        ...
        case SUSPENSE_TREE_OPERATION_SUSPENDERS: {
          ...
          for (let changeIndex = 0; changeIndex < changeLength; changeIndex++) {
            ...
            if (__DEBUG__) {
              ...
              debug(
                "Suspender changes",
                `Suspense node ${id} unique suspenders set to ${
  ...
  onBackendReloadAndProfileSupported = (isReloadAndProfileSupported) => {
  ...
  onHostInstanceSelected = (elementId) => {
